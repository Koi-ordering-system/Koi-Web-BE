...
public class GetUnapprovedTrips
{
    ...
    public record GetUnapprovedTripsRequest(
        int pageIndex = 1,
        int pageSize = 10,
        string keyword = "",
        Guid? farmId = null,
    ...
    {
        public static Response FromEntity(Trip trip) =>
            new(
                Id: trip.Id,
                FarmId: trip.FarmId,
                FarmName: trip.Farm.Name,
                FarmImages: trip.Farm.FarmImages.Select(i => i.Url).ToArray(),
                KoiDetails: trip.Farm.FarmKois.Select(fk => new KoiDetails(fk.KoiId, fk.Koi.Name))
                    .ToArray(),
                Days: trip.Days,
                Price: trip.Price
            );
    ...
    public record KoiDetails(Guid Id, string Name);

    public class Handler(IApplicationDbContext context)
        : IRequestHandler<Query, Result<PaginatedList<Response>>>
    {
        public async Task<Result<PaginatedList<Response>>> Handle(
            Query request,
            CancellationToken cancellationToken
        )
        {
            IQueryable<Trip> query = context
                .Trips.AsNoTracking()
                .Include(t => t.Farm)
                .ThenInclude(f => f.FarmImages)
                .Include(t => t.Farm)
                .ThenInclude(f => f.FarmKois)
            ...
            if (request.StartDate is not null && request.EndDate is not null)
                query = query.Where(t =>
                    t.Days == (request.EndDate - request.StartDate)!.Value.Days
                );
            ...
            return Result<PaginatedList<Response>>.Succeed(
                new(gettingTrips.ToList(), total, request.PageIndex, request.PageSize)
            );
        }
    }
    ...
    public class Endpoint : IEndpoints
    {
        ...
        {
            app.MapGet("/api/trips/unapproved", Handle)
                .WithTags("Trips")
                .WithMetadata(new SwaggerOperationAttribute("Get all unapproved Trips"))
                .CacheOutput(b => b.Tag("Trips"));
        }

        public static async Task<IResult> Handle(
            ISender sender,
            [AsParameters] GetUnapprovedTripsRequest request,
            CancellationToken cancellationToken = default
        )
        {
            Result<PaginatedList<Response>> response = await sender.Send(
                new Query(
                    request.pageIndex,
                    request.pageSize,
                    request.keyword,
